...
Automated results: **42 tests passed, 0 failed** in workflow-test-results.txt (Node 24.19.0, 33.29 seconds). Actual session examples: sessions/. A two-hour-plus, six-stream virtual-time accounting test covers 720,001 pulse events; it is not a two-hour wall-clock recording or throughput test. Small real-time image recording and decode tests are reported separately. The operator UI was exercised in the in-app browser through configuration, precheck, arm, recording, completed-session review and actual saved-frame decoding; the decoded image was visually checked.
...
The Node image generator and metadata coordinator remain a software test harness. A separate pypylon path now performs bounded free-run recording from one to six selected physical cameras without TTL. Electrical triggering and native TDT event acquisition remain unimplemented pending the established lab gates. There is no claim of synchronized exposure, 100 fps color-camera acceptance, full-size six-camera throughput, two-hour endurance, calibrated timing tolerance, power-loss durability, or completed in-app neural preview.
...
The web application now records one to six physical cameras concurrently without TTL pulses. It preserves native pixels, block IDs, camera timestamps, PC receive times, and per-frame hashes, writes FFV1/MKV, decodes every saved frame, and restores the original frame-rate and trigger settings. Two color cameras have completed a 30 fps, 60-second recording on the development workstation.

Live preview now targets 10 fps. Conversion and downsampling run on an independent latest-frame worker so delayed preview work does not block capture. Hardware exposure synchronization, native TDT events, long-duration segmentation, and real neural traces remain unimplemented or unaccepted.
