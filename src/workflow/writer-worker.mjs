...
const frameFD = fs.openSync(frameFile, 'wx');
fs.writeSync(frameFD, frameColumns.join(',') + '\n');
...
function openSegment(index) {
  const part = segments.length + 1,
    name = `camera_${camera.id}_part${String(part).padStart(5, '0')}.mkv`;
  segment = {
    file: name,
    hash_file: name + '.hashes.jsonl',
    part,
  ...
  encode = launch(ffmpeg, [
    ...
    'rawvideo',
    '-pixel_format',
    FORMATS[camera.pixelFormat].ffmpeg,
    '-video_size',
...
async function write(frame) {
  ...
  if (config.scenario === 'writer-failure' && total >= config.faultPulse)
    throw new Error('INJECTED_WRITER_FAILURE');
  if (
    segment &&
    Math.floor(frame.index / config.fps / config.segmentSeconds) !==
      Math.floor(segment.first_sim_index / config.fps / config.segmentSeconds)
  )
    await closeSegment();
  if (!segment) openSegment(frame.index);
  const b = Buffer.from(frame.pixels),
    sha256 = hash(b),
    videoIndex = segment.frames;
  await new Promise((resolve, reject) => encode.stdin.write(b, (e) => (e ? reject(e) : resolve())));
  const row = {
    session_id: sessionId,
    camera_serial: '',
    video_part: segment.file,
    video_frame_index: videoIndex,
    camera_block_id: '',
    camera_timestamp_ticks: '',
    camera_tick_hz: '',
    pc_receive_monotonic_ns: frame.pcNs,
    record_status: 'submitted_to_encoder',
    frame_width: camera.width,
    frame_height: camera.height,
    sim_camera_id: camera.id,
    sim_event_id: frame.eventId,
    sim_block_id: frame.block,
    sim_camera_ticks: frame.cameraTicks,
    clock_domain: `sim_camera_${camera.id}`,
    sha256,
    pixel_format: camera.pixelFormat,
  };
  ...
  segment.frames++;
  total++;
  send({ type: 'written', index: frame.index, part: segment.file, videoIndex });
}
async function fault(e) {
  if (failed) return;
  failed = true;
  send({ type: 'fault', error: e.message });
  try {
    await closeSegment();
  } catch (closeError) {
    send({ type: 'fault', error: closeError.message });
  }
}
  ...
  chain = chain
    .then(async () => {
      if (message.type === 'frame') {
        if (!failed) await write(message.frame);
        ...
        try {
          ...
          for (const s of segments)
            if (s.encoded) {
              try {
                s.verification = await verifyVideo(
                  ffmpeg,
                  path.join(dir, s.file),
                  path.join(dir, s.hash_file),
                  camera.pixelFormat,
                );
                s.verified = true;
              } catch (e) {
                s.verification_error = e.message;
                failed = true;
                send({ type: 'fault', error: e.message });
              }
            }
        ...
        send({ type: 'finished', total, segments, failed });
        parentPort.close();
      }
    })
    .catch(fault);
